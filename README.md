...
无尽书证是一个多租户、Waline 兼容的托管评论 SaaS。用户注册后可创建独立评论实例，并把任意现有 Waline 客户端的服务地址替换为 `https://{instance}.waline.infvar.com` 继续使用。

## 功能

- Waline 兼容 API：评论列表、发布、回复、点赞、用户信息、文章统计
- 多租户实例：`{instance}.waline.infvar.com`，共享数据库逻辑隔离
